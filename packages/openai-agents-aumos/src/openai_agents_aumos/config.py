...
"""
Configuration for openai-agents-aumos integration.

All runtime behaviour of the integration is controlled through ``GuardrailConfig``.
...
class GuardrailConfig(BaseModel):
    """
    Configuration for the AumOS governance guardrail with OpenAI Agents SDK.

    Pass an instance of this to ``AumOSGuardrail`` to control how governance
    decisions affect tool execution in OpenAI Agents SDK agents.

    Attributes:
        agent_id: Identifier for the agent being governed.  Used in all
            evaluation requests and audit records.
        on_denied: What to do when a governance evaluation returns a denial.
            ``'raise'`` — raise ``GovernanceDeniedError`` (default).
            ``'skip'`` — return a denial message as the guardrail output.
            ``'log'`` — log the denial at WARNING level and allow execution.
        ...
        scope_mapping: Optional mapping from tool name to governance scope string.
            ...
            ``default_scope``.
        amount_field: If set, the integration will attempt to read this key from
            the tool input dict and pass it as the spend amount to the governance
            evaluation.  If the field is absent, the amount is omitted.
        audit_all_calls: When True, record an audit event for every completed
            tool call.  When False, only denied calls are audited.  Default True.
    ...
    amount_field: str | None = Field(
        default=None,
        description=(
            "Dict key to extract as the spend amount from tool inputs. "
