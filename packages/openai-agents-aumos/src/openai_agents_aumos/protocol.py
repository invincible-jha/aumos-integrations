...
satisfy when used with openai-agents-aumos.  Using a Protocol instead of ``Any``
allows mypy --strict to verify that the engine instance passed to the integration
actually exposes the methods this package calls: ``evaluate_sync`` and
``record_audit_event``.
...
class GovernanceAction(TypedDict, total=False):
    ...
    ~~~~~~~~~~~~~~~
    agentId
        Identifier for the OpenAI Agents SDK agent requesting the action.
    action
...
class GovernanceEngineProtocol(Protocol):
    ...
    Any object passed to openai-agents-aumos as an ``engine`` must expose at
    ...
    evaluate_sync(agent_id, scope, amount)
        Synchronous governance evaluation used inside ``AumOSGuardrail``
        before tool execution is permitted.
