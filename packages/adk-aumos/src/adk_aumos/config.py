...
"""
Configuration for adk-aumos integration.
...
class GovernanceConfig(BaseModel):
    """
    Configuration for the AumOS governance integration with Google ADK.

    Pass an instance of this to ``AumOSADKCallback`` to control how governance
    decisions affect tool execution in Google ADK agents.

    Attributes:
        agent_id: Identifier for the ADK agent being governed.  Used in all
        ...
        on_denied: What to do when a governance evaluation returns a denial.
            ...
            ``'skip'`` — log the denial and return without raising; the ADK
                agent receives no output for that tool call.
        ...
        amount_field: If set, the integration will attempt to read this key from
            ...
            evaluation.  If the field is absent or the input is not a dict, the
            amount is omitted.
        audit_all_calls: When True, record an audit event for every completed tool
            call.  When False, only denied calls are audited.  Defaults to True.
    ...
    agent_id: str = Field(
        default="default",
        min_length=1,
        description="Identifier for the ADK agent being governed.",
    )
