...
satisfy when used with langchain-aumos.  Using a Protocol instead of ``Any`` allows
mypy --strict to verify that the engine instance passed to the integration actually
exposes the two methods this package calls: ``evaluate_sync`` and ``record_audit_event``.
...
.. code-block:: python
    ...
    from langchain_aumos.protocol import GovernanceEngineProtocol, GovernanceAction

    def build_callback(engine: GovernanceEngineProtocol) -> AumOSGovernanceCallback:
        return AumOSGovernanceCallback(engine)
"""
...
class GovernanceAction(TypedDict, total=False):
    ...
    ~~~~~~~~~~~~~~~
    agentId
        Identifier for the agent requesting the action.
    action
        Free-text name of the action being evaluated (e.g. ``'tool:web_search'``).
    category
...
class GovernanceDecision(TypedDict, total=False):
    ...
    permitted
        ...
        denied.  The integration uses this field to gate tool execution.
    reason
...
class GovernanceEngineProtocol(Protocol):
    ...
    Any object passed to langchain-aumos as an ``engine`` must expose at
    ...
    evaluate_sync(agent_id, scope, amount)
        Synchronous governance evaluation used inside
        ``AumOSGovernanceCallback.on_tool_start``.  LangChain callbacks are
        synchronous so the engine must provide a non-async evaluation path.

    record_audit_event(agent_id, tool_name, run_id, succeeded, ...)
        Append-only audit recording called after a tool completes or fails.
        Audit logging is RECORDING ONLY — the engine must not trigger side
        effects beyond persisting the event.
    """
    ...
    def record_audit_event(
        ...
        *,
        ...
        Args:
            ...
            run_id: LangChain run identifier for correlation.
